...
if( !class_exists( 'wpForo' ) ) {
	...
	final class wpForo{
		...
		public function statistic( $mode = 'get', $template = 'all' ){
			...
			if( $mode == 'get' ){
				$user_groupid = ( $this->current_user_groupid ) ? $this->current_user_groupid : 4;
				if( $cached_stat = get_option('wpforo_stat_' . $user_groupid ) ){
			...
			if( $mode == 'get' || $template == 'all' ) {
                ...
                $stats = get_wpf_option('wpforo_stat_' . $this->current_user_groupid, $this->default->stats);
                switch ($template){
                    case 'forum':
                        $stats['forums'] = $this->forum->get_count( array('is_cat' => 0) );
                    break;
			...
			$user_groupid = ( $this->current_user_groupid ) ? $this->current_user_groupid : 4;
            if( isset($user_groupid) && $user_groupid ){
				update_option( 'wpforo_stat_' . $this->current_user_groupid, $stats );
			}
            return $stats;
        }
