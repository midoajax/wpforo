	                ...
	                <?php $bg = FALSE; foreach($members as $member) : ?>
	                  ...
	                  <tr<?php echo ( $bg ? ' style="background:#F7F7F7"' : '' ) ?>>
	                    ...
	                    <td class="wpf-members-info">
	                        ...
	                        <span class="wpf-member-info wpfcl-1"> <i class="fa fa-users" title="<?php wpforo_phrase('Usergroup') ?>"></i>&nbsp; <?php wpforo_phrase($member['groupname']) ?> | <?php if( wpforo_feature('rating') && $enabled_for_usergroup ): ?><i class="fa fa-star" title="<?php wpforo_phrase('Rating') ?>"></i>&nbsp;<?php echo WPF()->member->rating_level( $member['posts'], FALSE ) ?>/10  |<?php endif; ?> <?php wpforo_phrase('Posts') ?>: <?php echo intval($member['posts']) ?></span>
